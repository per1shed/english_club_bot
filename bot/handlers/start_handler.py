    ...
    if query.data == "why_club":
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton("Оплатить зарубежной картой", callback_data="pay_foreign"),
            ],
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Завершение диалога"""
    await update.message.reply_text(
        "Диалог завершен. Для начала нажмите /start",
        reply_markup=None
    )
    return ConversationHandler.END


# Если нужно оставить старую функцию как fallback
async def simple_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Простой старт (можно удалить если не нужен)"""
    await update.message.reply_text(
        "Привет! Используйте /start для запуска основного меню клуба."
    )